...
extends Node
## Generates Consciouness Core every X amount of stardust being created.

## Amount of stardust required to create the next consciounes core.
...
func initialize_new_milestone(transfered_progress : int = 0) -> void:
	if universe.consciouness_core == 0:
		stardust_goal = 4
	
	else:
		stardust_goal = universe.consciouness_core * 8
...
func check_for_completion() -> void:
	...
	HandlerConsciounessCore.ref.create_consciouness_core(1)
